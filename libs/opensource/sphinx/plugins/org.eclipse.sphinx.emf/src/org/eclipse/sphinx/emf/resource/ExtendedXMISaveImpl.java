 ...
 *   See4sys - copied from org.eclipse.emf.ecore.xmi.impl.XMISaveImpl 
 *             for letting it extend {@link ExtendedXMLSaveImpl} instead 
 *             of {@link XMLSaveImpl}
 *
 * </copyright>
 */
...
public class ExtendedXMISaveImpl extends ExtendedXMLSaveImpl {
	...
	protected static final String XMI_ID_NS = XMIResource.XMI_NS + ":" + XMIResource.XMI_ID; // xmi:id //$NON-NLS-1$
	protected static final String XMI_TAG_NS = XMIResource.XMI_NS + ":" + XMIResource.XMI_TAG_NAME; // xmi:XMI //$NON-NLS-1$
	protected static final String XMI_TYPE_NS = XMIResource.XMI_NS + ":" + XMLResource.TYPE; // xmi:type //$NON-NLS-1$
	protected static final String XMI_VER_NS = XMIResource.XMI_NS + ":" + XMIResource.VERSION_NAME; // xmi:version //$NON-NLS-1$
	protected static final String XMI_XMLNS = XMLResource.XML_NS + ":" + XMIResource.XMI_NS; // xmlns:xmi //$NON-NLS-1$

	public ExtendedXMISaveImpl(XMLHelper helper) {
		super(helper);
		idAttributeName = XMI_ID_NS;
		idAttributeNS = XMIResource.XMI_NS;
	}

	public ExtendedXMISaveImpl(Map<?, ?> options, XMLHelper helper, String encoding) {
		this(options, helper, encoding, "1.0"); //$NON-NLS-1$
	}

	public ExtendedXMISaveImpl(Map<?, ?> options, XMLHelper helper, String encoding, String xmlVersion) {
		super(options, helper, encoding, xmlVersion);
		xmiType = Boolean.TRUE.equals(options.get(XMIResource.OPTION_USE_XMI_TYPE));
		idAttributeName = XMI_ID_NS;
		idAttributeNS = XMIResource.XMI_NS;
	}
