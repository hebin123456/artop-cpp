...
public class SinglePackageNodelModelBuilder extends AbstractNodeModelBuilder {

	public SinglePackageNodelModelBuilder(String name, int persistenceMappingStrategy) {
	...
	protected List<EPackage> createMetaModel(String name, boolean isMany, int persistenceMappingStrategy) {
		...
		EPackage ePackage = createEPackage(name, "http://www.eclipse.org/sphinx/tests/emf/serialization/generators/1", "nodes1");
		ePackages.add(ePackage);

		EClass nodeEClass = createEClass(ePackage, "Node", "NODE", "NODES", true);
		createEReference(nodeEClass, "children", true, isMany, nodeEClass, "CHILD", "CHILDREN", persistenceMappingStrategy);
