...
emf::ecore::EClass* XSDMaxLengthFacet::eClass() const {
    return XSDPackage::instance().getEClass_XSDMaxLengthFacet();
}

std::any XSDMaxLengthFacet::eGet(const emf::ecore::EStructuralFeature* feature) const {
...
void XSDMaxLengthFacet::eSet(const emf::ecore::EStructuralFeature* feature, std::any value) {
    auto* ef = asEStructuralFeature(feature);
    if (!ef) return;
    const std::string& n = ef->getName();
...
bool XSDMaxLengthFacet::eIsSet(const emf::ecore::EStructuralFeature* feature) const {
    auto* ef = asEStructuralFeature(feature);
    if (!ef) return false;
    const std::string& n = ef->getName();
...
void XSDMaxLengthFacet::eUnset(const emf::ecore::EStructuralFeature* feature) {
    auto* ef = asEStructuralFeature(feature);
    if (!ef) return;
    const std::string& n = ef->getName();
    if (n == "value") value_ = -1;
