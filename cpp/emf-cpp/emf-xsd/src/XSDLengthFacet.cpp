...
emf::ecore::EClass* XSDLengthFacet::eClass() const {
    return XSDPackage::instance().getEClass_XSDLengthFacet();
}

std::any XSDLengthFacet::eGet(const emf::ecore::EStructuralFeature* feature) const {
...
void XSDLengthFacet::eSet(const emf::ecore::EStructuralFeature* feature, std::any value) {
    auto* ef = asEStructuralFeature(feature);
    if (!ef) return;
    const std::string& n = ef->getName();
...
bool XSDLengthFacet::eIsSet(const emf::ecore::EStructuralFeature* feature) const {
    auto* ef = asEStructuralFeature(feature);
    if (!ef) return false;
    const std::string& n = ef->getName();
...
void XSDLengthFacet::eUnset(const emf::ecore::EStructuralFeature* feature) {
    auto* ef = asEStructuralFeature(feature);
    if (!ef) return;
    const std::string& n = ef->getName();
    if (n == "value") value_ = -1;
