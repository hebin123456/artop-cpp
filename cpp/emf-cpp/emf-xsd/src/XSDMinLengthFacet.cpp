...
emf::ecore::EClass* XSDMinLengthFacet::eClass() const {
    return XSDPackage::instance().getEClass_XSDMinLengthFacet();
}

std::any XSDMinLengthFacet::eGet(const emf::ecore::EStructuralFeature* feature) const {
...
void XSDMinLengthFacet::eSet(const emf::ecore::EStructuralFeature* feature, std::any value) {
    auto* ef = asEStructuralFeature(feature);
    if (!ef) return;
    const std::string& n = ef->getName();
...
bool XSDMinLengthFacet::eIsSet(const emf::ecore::EStructuralFeature* feature) const {
    auto* ef = asEStructuralFeature(feature);
    if (!ef) return false;
    const std::string& n = ef->getName();
...
void XSDMinLengthFacet::eUnset(const emf::ecore::EStructuralFeature* feature) {
    auto* ef = asEStructuralFeature(feature);
    if (!ef) return;
    const std::string& n = ef->getName();
    if (n == "value") value_ = -1;
