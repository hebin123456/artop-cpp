// AdapterFactoryEmitter: 生成 <Pkg>AdapterFactory.h/.cpp
...
public:
    AdapterFactoryEmitter(emf::ecore::EPackage* package, const std::string& baseNamespace,
                          const std::string& parentPath = std::string{});

    std::string emitHeader() const;
    std::string emitSource() const;

    std::string className() const { return capitalizeFirst(package_->getName()) + "AdapterFactory"; }
